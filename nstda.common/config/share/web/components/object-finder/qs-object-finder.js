...
(function()
{
   ...
   YAHOO.extend(Alfresco.ObjectFinder, Alfresco.component.Base,
   {
      ...
      options:
      {
         ...
         // qs
          /**
          * Determines if an "Upload" button shall be displayed
          *
          * @property allowUploadAction
          * @type boolean
          * @default false
          */
      ...
      onReady: function ObjectFinder_onReady()
      {
         ...
         if (!this.options.disabled)
         {
            ...
            if (itemGroupActionsContainerEl)
            {
			   ...
			   //qs
			   if (this.options.allowUploadAction) {
					//if (this.options.field == "assoc_isowf_sourceDocument" || this.options.field == "assoc_isowf_publishDocument") {
				  //if (this.options.field == "assoc_memwf_attachDocument" || this.options.field == "assoc_memwf_memoDocument"
					//   || this.options.field == "assoc_cpwf_attachDocument") {
				     var uploadButtonEl = document.createElement("button");
                     itemGroupActionsContainerEl.appendChild(uploadButtonEl);

                     this.widgets.uploadButton = Alfresco.util.createYUIButton(this, null, this.onUploadButtonClick,
                     {
                        label: this.msg("button.upload"),
                        disabled: false
                     }, uploadButtonEl);
                //  }
               }
			   ...
			   // upload new version
			   if (this.options.allowUploadNewAction) {
					//if (this.options.field == "assoc_memwf_attachDocument" || this.options.field == "assoc_memwf_memoDocument"
					//	|| this.options.field == "assoc_cpwf_attachDocument") {

						 var uploadNewButtonEl = document.createElement("button");
						 itemGroupActionsContainerEl.appendChild(uploadNewButtonEl);

						 this.widgets.uploadNewButton = Alfresco.util.createYUIButton(this, null, this.onUploadNewButtonClick,
						 {
							label: this.msg("button.upload.new"),
							disabled: false
						 }, uploadNewButtonEl);
					//}
			   }
			   ...
			   // gen doc
               if (this.options.allowGenDocAction) {
				  //if (this.options.field == "assoc_isowf_sourceDocument" || this.options.field == "assoc_isowf_publishDocument"
				//	  || this.options.field == "assoc_cpwf_attachDocument") {
					 var genDocButtonEl = document.createElement("button");
					 itemGroupActionsContainerEl.appendChild(genDocButtonEl);

					 this.widgets.genDocButton = Alfresco.util.createYUIButton(this, null, this.onGenDocButtonClick,
					 {
						label: this.msg("button.genDoc"),
						disabled: false
					 }, genDocButtonEl);
				 // }
               }
			   ...
			   // watermark
			   if (this.options.allowWatermarkAction) {
				   var watermarkButtonEl = document.createElement("button");
				   itemGroupActionsContainerEl.appendChild(watermarkButtonEl);

				   this.widgets.watermarkButton = Alfresco.util.createYUIButton(this, null, this.onWatermarkButtonClick,
				   {
					  label: this.msg("button.watermark"),
					  disabled: false
				   }, watermarkButtonEl);
			   }

               // download
               if (this.options.allowDownloadAction) {
				   var downloadButtonEl = document.createElement("button");
                   itemGroupActionsContainerEl.appendChild(downloadButtonEl);

                   this.widgets.downloadButton = Alfresco.util.createYUIButton(this, null, this.onDownloadButtonClick,
                   {
                      label: this.msg("button.download"),
                      disabled: false
                   }, downloadButtonEl);
               }
            ...
            //Doy+ {
			if (this.options.allowUploadItemAction && (this.options.displayMode == "list" || this.options.displayMode == "items"))
			{
	            this.options.listItemActions.push(
	            {
	               name: "upload-list-item",
	               event: "uploadListItem",
			...
			//qs
			if (this.options.allowRemoveAction && (this.options.displayMode == "list" || this.options.displayMode == "items"))
            {
               this.options.listItemActions.push(
               {
      ...
      onUploadNewButtonClick: function ObjectFinder_onFileUpload(e, p_obj)
      {
     	 this.updating = false;
         if (this.fileUpload === null)
         {
...
//		var target = this.options.targetFolder;

		 var srcNodeRef = this.options.srcNodeRef;
		 
		 var me = this;
		 
		 params = "?pdfNodeRef="+srcNodeRef;
    	 url = appContext + "/proxy/alfresco/getVersion"+params;
    	 myCallback = {
    	   success: function(o) {
    		   var version = o.responseText;
    		   
    		   var uploadConfig =
    	         {
    				//destination: folderNodeRef,
    				filter: [],
    	            mode: me.fileUpload.MODE_SINGLE_UPDATE,
    	            thumbnails: "doclib",
    				controlId:me.options.controlId,
    				updateNodeRef:srcNodeRef,
    				updateVersion:version,
    	            onFileUploadComplete:
    	            {
    	               fn: me.onFileUploadComplete,
    	               scope: me
    	            }
    	         }; 
      	       ...
      	       me.fileUpload.show(uploadConfig);
	      	     ...
	      	     if (YAHOO.lang.isArray(p_obj) && p_obj[1].tooltip)
	             {
	                var balloon = Alfresco.util.createBalloon(me.fileUpload.uploader.id + "-dialog",
	                {
	                   html: p_obj[1].tooltip,
	                   width: "30em"
	                });
	                ...
	                me.fileUpload.uploader.widgets.panel.hideEvent.subscribe(function()
	                {
	                   balloon.hide()
	                });
	             } 
    	   ...
    	   failure: function(o) {alert("Get Version failed")},
    	 };
	  ...
	  onDownloadButtonClick: function ObjectFinder_onFileUpload(e, p_obj)
      {
		  var nodeRef = this.options.srcNodeRef;
      	  window.location= appContext+"/proxy/alfresco/download?nodeRef=" + nodeRef;
      },
      
      onGenDocButtonClick: function ObjectFinder_onFileUpload(e, p_obj)
      {
		  var nodeRef = this.options.srcNodeRef;
    	  var docNumber =  document.getElementById(el+'prop_isowf_docNumber');
    	  if (docNumber) {
	      	  var el = this.id.split("assoc")[0];
		      var effectiveDate = document.getElementById(el+'prop_isowf_effectiveDate');
		      var prepareDate = document.getElementById(el+'prop_isowf_requestDate');
		      var dateField = YAHOO.util.Dom.get(el+'prop_isowf_effectiveDate'+'-cntrl-date');
		      var prepare_dateField = YAHOO.util.Dom.get(el+'prop_isowf_requestDate'+'-cntrl-date');
		      if(docNumber.value == null || docNumber.value == undefined || docNumber.value == ''){
		    	  docNumber.focus();
		    	  return false;
		      }else if(effectiveDate.value == null || effectiveDate.value == undefined || effectiveDate.value == ''){
		    	  dateField.focus();
		    	  return false;
		      }
		      else if(prepareDate.value == null || prepareDate.value == undefined || prepareDate.value == ''){
		    	  prepare_dateField.focus();
		    	  return false;
		      }
		      
	      	  window.location=appContext+"/proxy/alfresco/gendoc?nodeRef=" + nodeRef+"&n="+docNumber.value+"&e="+effectiveDate.value.split("T")[0]+"&p="+prepareDate.value.split("T")[0];
  
      	  } else {
			  docNumber = this.options.docNumber;
		      var effectiveDate = this.options.effectiveDate;
	      	  window.location= appContext+"/proxy/alfresco/gendoc?nodeRef=" + nodeRef;
      	  }
      },

      onWatermarkButtonClick: function ObjectFinder_onFileUpload(e, p_obj)
      {
		var folderPdfNodeRef = this.options.pdfTempFolderNodeRef;
      	if (this.fileUpload === null)
        {
			// this.fileUpload = Alfresco.getFileUploadInstance();
			this.fileUpload = Alfresco.util.ComponentManager.findFirst("Alfresco.FlashUpload");
  		...
  		var pos = this.id.indexOf("-cntrl");
		var newId = this.id.substring(0,pos);

		var oldNodeRef = document.getElementById(newId).value;
        ...
        if (oldNodeRef != "") {
        	   ...
        	   success: function(o) {
        		   var version = o.responseText;
	      	       ...
	      	       uploadConfig =
	    	         {
	                    ...
	                    thumbnails: "doclib",
	        			controlId:me.options.controlId,
	        			updateNodeRef:oldNodeRef,
	        			updateVersion:version,
	        			overwrite:true,
	                    onFileUploadComplete:
	                    {
	     	               fn: me.onFileUploadWatermarkComplete,
	                       scope: me
	                    }
	    	       };  
	         ...
	         uploadConfig =
	         {
	            ...
	            onFileUploadComplete:
	            {
	               fn: this.onFileUploadWatermarkComplete,
	               scope: this
	            }
	         };
      ...
      onFileUploadWatermarkComplete:function(complete)
      {
         var success = complete.successful.length;
         if (success != 0)
         {
        	 var el = this.id.split("assoc")[0];
        	 var isGenWatermark = false;
        	 var isGenSign =  document.getElementById(el+'prop_isowf_isGenSign').value;
             var noderef = complete.successful[0].nodeRef;
             var srcNodeRef = this.options.srcNodeRef;
             var parentNodeRef =  document.getElementsByName('prop_isowf_pdfNodeRef')[0].value;

        	 var params = "?pdfNodeRef="+noderef+"&srcNodeRef=" + srcNodeRef+"&isGenWatermark="+false+"&isGenSign="+isGenSign+"&parentNodeRef="+parentNodeRef;
        	 var url = appContext + "/proxy/alfresco/watermark"+params;
        	 var myCallback = {
        	   success: function(o2) {
	              	 //params = "?pdfNodeRef="+noderef;
	            	 url = appContext + "/proxy/alfresco/watermarkFinal"+params;
	            	 var myCallback2 = {
	            	   success: function(o) {
						   if (el) {
							   if (o.responseText == "false") {
								  ...
								  setTimeout(YAHOO.util.Connect.asyncRequest('GET', url, myCallback3),5000);
								}
								else {
									if(o2.responseText== "false"){
										alert("Sign failed")
									}else{
										alert("Finish");
									}
	            	 ...
	            	 setTimeout(YAHOO.util.Connect.asyncRequest('GET', url, myCallback2),1000);
	           },
        	   failure: function(o) {alert("Sign failed")},
        	 };
        	 //var transaction = YAHOO.util.Connect.asyncRequest(
        	 //  'GET', url, myCallback);
        	 
        	 setTimeout(YAHOO.util.Connect.asyncRequest('GET', url, myCallback),5000);
        	 
//            window.location="/share/proxy/alfresco/watermark?pdfNodeRef="+noderef+"&srcNodeRef=" + srcNodeRef;
         }
	  },
                  ...
                  if (items.hasOwnProperty(key))
                  {
                     ...
                     if (this.options.showLinkToTarget && this.options.targetLinkTemplate !== null)
                     {
                        if (this.options.displayMode == "items")
                        {
                           ...
                           {
							   link = YAHOO.lang.substitute(linkTemplate,this.options.targetLinkTemplate, item);
...
//                              });
      ...
      onSelectedItemAdded: function ObjectFinder_onSelectedItemAdded(layer, args)
      {
         ...
         if ($hasEventInterest(this, args))
         {
            ...
            {
			   var table;
			   if (this.options.allowUploadAction || this.widgets.currentValuesDataTable) {
					table = this.widgets.currentValuesDataTable;
			   }
			   else {
               ...
               if (i == il)
               {
                  ...
                  if (obj.highlight)
                  {
					 ...
					 var dataTableEl = table.get("element");
                     dataTableEl.scrollTop = dataTableEl.scrollHeight;
                     Alfresco.util.Anim.pulse(table.getLastTrEl());
                  }
               }
               else
               {
      ...
      /**
       * Upload selected item from datatable used in "list" mode
       *
       * @method onUploadListItem
       * @param layer {object} Event fired (unused)
       * @param args {array} Event parameters
       */
      onUploadListItem: function ObjectFinder_onUploadListItem(event, args)
      {
       	 var me = this;
	             ...
	             params = "?nodeRef="+oldNodeRef;
	          	 ...
	          	 myCallback = {
	          	   success: function(o) {
		      	       ...
		      	       uploadConfig =
		    	       {
		      	    		updateFilename:'document',
		         			filter: [],
		                    mode: me.fileUpload.MODE_SINGLE_UPDATE,
		                    thumbnails: "doclib",
		        			controlId:me.options.controlId,
		        			uploadURL:me.options.uploadURL,
		        			jobNo:me.options.jobNo,
		        			updateNodeRef:oldNodeRef,
		        			updateVersion:version,
		        			overwrite:true,
		                    onFileUploadComplete:
		                    {
		     	               fn: me.onFileUploadItemComplete,
		                       scope: me
		                    },
		                    showTemplate:me.options.showTemplate
		    	       };  
      ...
      onFileUploadItemComplete:function(complete)
      {
	       var success = complete.successful.length;
	       
//	       for(var a in complete.successful[0]) {
//	    	   console.log(a+":"+complete.successful[0][a]);
//	       }
...
//    	  console.log(Alfresco.util.PopupManager.curNodeRef);
      ...
      onUploadFinished: function ObjectFinder_onUploadFinished(event, args)
      {
         if ($hasEventInterest(this, args))
         {
              var jsonData = YAHOO.lang.JSON.parse(args[1].uploadData);

              var item = {};
              item.description = jsonData.description;
              item.displayPath = jsonData.displayPath;
              item.isContainer = jsonData.isContainer;
              item.creator = jsonData.creator;
              item.modified = jsonData.modified;
              item.modifier = jsonData.modifier;
              item.name = jsonData.fileName;
              item.nodeRef = jsonData.nodeRef;
              item.selectable = jsonData.selectable;
              item.title = jsonData.selectable;
...
//                  });
               ...
               title = '<a target="_new" href="' + link + '">' + $html(item.name) + '</a>';
            }
         ...
         return function ObjectFinder_fnRenderCellListItemActions(elCell, oRecord, oColumn, oData)
         {
            ...
            if (scope.options.disabled === false) 
            {
               ...
               for (var i = 0, il = scope.options.listItemActions.length; i < il; i++)
               {
				  ...
				  // qs
                  if ( (listAction.name == "remove-list-item" && oRecord._oData.creator == Alfresco.constants.USERNAME) || (listAction.name != "remove-list-item")) {
      ...
      _enableActions: function ObjectFinder__enableActions()
      {
		 ...
		 if (this.widgets.uploadButton)
         {
            // Enable the add button
            this.widgets.uploadButton.set("disabled", false);                
         }
         if (this.widgets.uploadNewButton)
         {
            // Enable the add button
            this.widgets.uploadNewButton.set("disabled", false);                
         }
         if (this.widgets.watermarkButton)
         {
            // Enable the add button
            this.widgets.watermarkButton.set("disabled", false);                
         }
         if (this.widgets.downloadButton)
         {
            // Enable the add button
            this.widgets.downloadButton.set("disabled", false);                
         }
         if (this.widgets.genDocButton)
         {
            // Enable the add button
            this.widgets.genDocButton.set("disabled", false);                
         }
