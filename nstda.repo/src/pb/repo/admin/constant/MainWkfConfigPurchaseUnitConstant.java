package pb.repo.admin.constant;


public class MainWkfConfigPurchaseUnitConstant {
	
	public static final String TABLE_NAME = AdminConstant.TABLE_EXT_PREFIX + "wkf_config_pi_unit";
	
    /*
     * Table Field Name
     */
	public static final String TFN_ID = "ID";
	public static final String TFN_DESCRIPTION = "DESCRIPTION";
	public static final String TFN_NAME = "NAME";
	public static final String TFN_ORG_ID = "ORG_ID";
	public static final String TFN_DOCTYPE_ID = "DOCTYPE_ID";

    /*
     * JSON Field Name
     */
	public static final String JFN_ID = "id";
	public static final String JFN_DESCRIPTION = "description";
	public static final String JFN_NAME = "name";
	public static final String JFN_ORG_ID = "org_id";
	public static final String JFN_DOCTYPE_ID = "doctype_id";
	
}
