		...
		AuthenticationUtil.runAs(new RunAsWork<String>() {
			public String doWork() throws Exception
			{
				...
				try {
					...
					PcmOrdModel model = pcmReqService.get(id.toString());
					Integer level = model.getWaitingLevel();
					ExecutionEntity executionEntity = ((ExecutionEntity)task.getExecution()).getProcessInstance();
					
					String curUser = authenticationService.getCurrentUserName();
					String taskKey = task.getTaskDefinitionKey();
					String action = task.getVariable(WF_PREFIX+"consultOutcome")!=null ? task.getVariable(WF_PREFIX+"consultOutcome").toString():null;
					
					executionEntity.setVariable(WF_PREFIX+"consultOutcome", action);
		...
		AuthenticationUtil.runAs(new RunAsWork<String>() {
			public String doWork() throws Exception
			{
				...
				try {
					...
					PcmOrdModel memoModel = pcmReqService.get(memoId.toString());
					...
					Boolean isLastReviewer = isLastReviewerAndNoCloseUser(level, memoModel, action);
...
//					}
					...
					if(completeOutcome.equals("Completed Workflow") || isLastReviewer) {
						...
						memoModel.setStatus(PcmOrdConstant.ST_CLOSED);
...
//						createPaperHistory(memoId.toString(), targetFoelderNodeRef.toString(), executionEntity);
					...
					if(reSubmitOutcome!=null && reSubmitOutcome.equalsIgnoreCase("Cancel")) {
						memoModel.setId(memoId.toString());
						memoModel.setStatus(PcmOrdConstant.ST_CANCEL);
						memoModel.setWaitingLevel(null);
					...
					PcmOrdWorkflowModel memoWorkflowModel = new PcmOrdWorkflowModel();
					...
					if(completeOutcome.equals("Completed Workflow") || isLastReviewer) {
						...
						JSONObject dataMap = PcmOrdUtil.convertToJSONObject(memoModel,false);
	...
	private void update(PcmOrdModel memoModel, Object au, Object ag) throws Exception {
	...
	private int getLastReviewerLevel(PcmOrdModel memoModel) {
		int lastLevel = 0;
		
		return lastLevel;
	}	
	
	private Boolean isLastReviewerAndNoCloseUser(Integer level, PcmOrdModel memoModel, String action) {
		List<MainMasterModel> wfList = adminMasterService.list(MainMasterConstant.TYPE_WORKFLOW, memoModel.getWorkflowId());
		if (!wfList.isEmpty()) {
	...
	String saveWorkflowHistory(DelegateExecution execution, String user, String stateTask, String taskComment, String action, DelegateTask task, Long workflowId, Integer level) throws Exception {
		...
		if(action.equalsIgnoreCase("Approve")){
			...
			action = "ยกเลิก";
		}else if(action.equalsIgnoreCase("Resubmit")){ /**Kay**/
			action = "ขออนุมัติ";
		}
		...
		PcmOrdWorkflowHistoryModel memoWorkflowHistoryModel = new PcmOrdWorkflowHistoryModel();
			 ...
			 public String doWork() throws Exception
			 	{
						...
						List<PcmOrdWorkflowHistoryModel> list = pcmWorkflowService.listHistory(params);
						//log.info("list :: " + list);
						List<Map<String, Object>> listHistory = new ArrayList<Map<String,Object>>();
						Map<String, Object> dataMap =null;
						
						for(PcmOrdWorkflowHistoryModel model : list) {
