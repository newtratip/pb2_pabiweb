...
@Component("pb.pcm.workflow.CompleteTask")
		...
		AuthenticationUtil.runAs(new RunAsWork<String>() {
			public String doWork() throws Exception
			{
				...
				try {
					...
					PcmReqModel memoModel = memoService.get(memoId.toString());
...
//						createPaperHistory(memoId.toString(), targetFoelderNodeRef.toString(), executionEntity);
					...
					if(completeOutcome.equals("Completed Workflow")) {
			    		...
			    		memoModel = memoService.genDoc(memoModel, folderNodeRef);
			    		memoService.update(memoModel);
