Ext.define('PBExpUse.controller.voyager.Form', {
    extend: 'Ext.app.Controller',

    refs:[{
    	ref:'empGrid',
		selector:'expUseVoyagerTab grid[itemId=empGrid]'
	},{
    	ref:'othGrid',
		selector:'expUseVoyagerTab grid[itemId=othGrid]'
    }],
    ...
    init:function() {
		...
		me.control({
			'expUseVoyagerTab grid [action=addEmp]': {
				click : me.addEmp
			},
			'expUseVoyagerTab grid [action=addOth]': {
				click : me.addOth
			},
			'expUseVoyagerTab grid':{
				editEmp:me.editEmp,
				delEmp:me.delEmp,
				editOth:me.editOth,
				delOth:me.delOth
	...
	DEL_MSG_KEY : 'DELETE_EXP_USE_VOYAGER',
    URL : ALF_CONTEXT+'/exp',
    ...
    dlgEmployeeUserCallBack:function(items) {
		...
		for(var a in items) {
			...
			var rec = Ext.create('PBExpUse.model.VoyagerGridModel',{
				id : maxId+1,
				action : 'D' 
			});
